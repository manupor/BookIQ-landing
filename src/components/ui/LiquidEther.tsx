...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      private updateHoverState(cx: number, cy: number) { this.isHoverInside = this.isPointInside(cx, cy); return this.isHoverInside; }
    ...
    class ExternalForce extends ShaderPass {
      ...
      update(...args: any[]) {
        const p = args[0] || {};
        const fx = (Mouse.diff.x / 2) * (p.mouse_force || 0); const fy = (Mouse.diff.y / 2) * (p.mouse_force || 0);
        const cs = p.cellScale || { x: 1, y: 1 }; const sz = p.cursor_size || 0;
        const csx = sz * cs.x; const csy = sz * cs.y;
        ...
        u.force.value.set(fx, fy); u.center.value.set(cx, cy); u.scale.value.set(sz, sz);
        super.update();
      }
    }
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: any[]) {
        ...
        for (let i = 0; i < (it ?? 0); i++) {
          ...
          if (this.uniforms) this.uniforms.pressure.value = pi.texture; this.props.output = po; super.update();
        }
        return po;
      }
    }
    ...
    class Pressure extends ShaderPass {
      ...
      update(...args: any[]) { const { vel, pressure: pr } = (args[0] || {}) as any; if (this.uniforms && vel && pr) { this.uniforms.velocity.value = vel.texture; this.uniforms.pressure.value = pr.texture; } super.update(); }
    }
    ...
    class WebGLManager implements LiquidEtherWebGL {
      props: any; output!: OutputClass; autoDriver?: AutoDriver;
      ...
      init() { if (!Common.renderer) return; this.props.$wrapper.prepend(Common.renderer.domElement); this.output = new OutputClass(); }
      ...
      dispose() {
        try { window.removeEventListener('resize', this._resize); if (this._onVisibility) document.removeEventListener('visibilitychange', this._onVisibility); Mouse.dispose();
